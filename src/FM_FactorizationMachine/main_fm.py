...
def fake_data(sample, dim, k):
    ...
    z = w0 + np.dot(w, x.T).T + np.longlong(
        np.sum((np.dot(x, v) ** 2 - np.dot(x ** 2, v ** 2)),  # ** 2 表示每个元素的平方
               axis=1).reshape(len(x), 1)) / 2.0  # axis=1表示行和
    y = sigmoid(z)
    data = np.concatenate((x, y), axis=1)  # axis=1表示按行拼接
    return z, data
